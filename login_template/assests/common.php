...
// this allows a new item to be added to a database
function new_item($conn, $post)//creates fuction
{
    try{// doing a prepared stament
        $sql = "INSERT INTO table name (feilds) VALUES(?,?,?,?,?)";//easy to sql attack
        $stmt = $conn->prepare($sql);//prepare sql

        $stmt->bindValue(1, $post['feild']);
        $stmt->bindValue(2, $post['feild']);
        $stmt->bindValue(3, $post['feild']);
        $stmt->bindValue(4, $post['feild']);
        $stmt->bindValue(5, $post['feild']);// binding the data from form to SQL statment this makes it more secure from a SQL injection attack less likly to hijk
...
// allows any messages to be formatted and outputted as well as returning nothing if theses an error
...
//this checks if the username is repeted in the table to make sure we dont have data redudency
...
//allows someone to register will add them to database
function reg_user($conn, $post)
{
    try{
        $sql = "INSERT INTO user (username, password, other feilds) VALUES(?,?,?)";//prepare for sqp quary
        $stmt = $conn->prepare($sql);//prepare sql
...
// this allows a user to log in this will check that there username was found and if not will return to login page
