...
//this allows the form to be used and the username to be checked
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {//checking a super globle to see if the request methord is post to call the page
    ...
    if (!username_check(dbconnect_insert(), $_POST["username"])) {//checks the value returned to see if username id avalible
        ...
        $_SESSION['usermessage'] = "User Registration: SUCCESSFUL";//gives and formats the resutle of the check from common username_check
    } else {
        $_SESSION['usermessage'] = "ERROR USER REGISTRATION FAILED ";//if its not aviblibe it prints this error message
    }
